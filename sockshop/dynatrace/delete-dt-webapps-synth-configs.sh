...
RESPONSE=$(curl -X GET -H "Content-Type: application/json; charset=utf-8" -H "Authorization: Api-Token $DT_CONFIG_TOKEN" $DT_API_URL/api/config/v1/autoTags)
echo $RESPONSE
...
for i in $(jq '.values | keys | .[]' <<< $RESPONSE); do
    ...
    echo $(curl -X DELETE -H  "accept: */*" -H  "Authorization: Api-Token $DT_CONFIG_TOKEN" "$DT_API_URL/api/config/v1/autoTags/$TAG_RULE_ID");
done
