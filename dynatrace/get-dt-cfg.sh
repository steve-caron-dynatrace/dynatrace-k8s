...
then
	...
	sed -i -r 's~DT_ENVIRONMENT_ID=(.*)~DT_ENVIRONMENT_ID\='"$DT_ENVIRONMENT_ID"'~' ../configuration.conf
	sed -i -r 's~DT_API_TOKEN=(.*)~DT_API_TOKEN\='"$DT_API_TOKEN"'~' ../configuration.conf
	sed -i -r 's~DT_PAAS_TOKEN=(.*)~DT_PAAS_TOKEN\='"$DT_PAAS_TOKEN"'~' ../configuration.conf
